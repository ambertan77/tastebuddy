...
type ButtonProps = {
    type: "green" | "logout" | "add" | "search" | "filter" | "no-bg" | "filter-clicked" | "added" ;
    size: "sm" | "med" | "big" ;
    text: string;
    onPress: () => void;
};
...
const ButtonTemplate = ({ type, size, text, onPress }: ButtonProps) => {
    const backgroundColour = type === "green" ? "bg-green-700" : type === "filter" ? "bg-transparent rounded-xl" : type === "filter-clicked" ? "bg-green-700 rounded-xl" : type === "logout" ? "bg-amber-600" : type === "added" ? "bg-green-700" : "bg-transparent";
    ...
    const border = type === "add" ? "border border-gray-500" : type === "added" ? "border border-green-700" : type === "filter" ? "border border-green-700" : type === "filter-clicked" ? "border border-green-700" : type === "search" ? "border border-white" : "";
    return (
        <TouchableOpacity
            onPress={onPress}
