...
describe('Favourites Page: Post Button 1', () => {  
    beforeEach(() => {
    ...
    it('Popup is initially closed', async () => {
        ...
        await waitFor(() => {
            expect(page.queryByTestId('reviewPopup')).toBeNull();
        })
    })

    it('Popup is open when Post button is pressed', async () => {
        fetchFood.mockResolvedValueOnce([
            {id: '1', Name: 'boiled egg', Price: '0.4', Nutrients: []},
            {id: '2', Name: 'egg tart', Price: '0.5', Nutrients: []},
            {id: '3', Name: 'chicken soup', Price: '3.2', Nutrients: []}
        ...
        await waitFor(() => userEvent.press(postButton))
        
        await waitFor(() => {
            expect(page.queryByTestId('reviewPopup')).toBeDefined();
        })
    })
});